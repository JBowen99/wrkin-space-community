	...
	for (const [taskId, color] of colorByTaskId) {
		...
		rules.push(
			`.wrkin-gantt-host .sg-task[data-task-id="${safeId}"]{` +
				`background-color:color-mix(in srgb, ${color} 16%, var(--color-surface-raised))!important;` +
				`color:var(--color-ink)!important;overflow:hidden;border-radius:8px!important;` +
				`border:1.5px solid color-mix(in srgb, ${color} 45%, var(--color-border))!important}` +
				`.wrkin-gantt-host .sg-task[data-task-id="${safeId}"] .sg-task-background{` +
				`background-color:color-mix(in srgb, ${color} 58%, var(--color-surface-raised))!important;opacity:1;border-radius:8px 0 0 8px}` +
				`.wrkin-gantt-host .sg-task[data-task-id="${safeId}"] .sg-task-content{` +
				`position:relative;z-index:1;color:var(--color-ink)!important}`
		);
	}
	return rules.join('');
}
