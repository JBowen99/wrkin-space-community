...
import { db } from './db';
import { taskAttachment, taskItem, wrkspaceModule } from './db/schema';
...
import { deleteObject, getObject, putObject, taskAttachmentKey } from './storage';
import { TASK_ATTACHMENT_MAX_BYTES } from '$lib/shared/task-attachments';
...
export async function userCanAccessTask(userId: string, taskId: string): Promise<boolean> {
	const { userCanAccessTaskById } = await import('./authorization');
