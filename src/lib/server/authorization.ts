...
/** Capabilities for a resolved access record — delete requires explicit wrkspace owner, not team admin. */
export function getWrkspaceCapabilitiesForAccess(access: WrkspaceAccess): WrkspaceCapabilities {
	const caps = getWrkspaceCapabilities(access.effectiveWrkspaceRole);
	return {
		...caps,
		delete_wrkspace: access.wrkspaceRole === 'owner'
	};
}
...
export function requireWrkspaceCapability(
	...
	const caps = getWrkspaceCapabilitiesForAccess(access);
	if (!caps[capability]) {
		error(403, 'Forbidden');
	}
}
