...
export async function recordActivity(input: ActivityEventInput): Promise<string | undefined> {
	...
	const recipientIds = resolveRecipientIds(input, catalog.notify, assigneeIds, wrkspaceMemberIds);
...
export async function listWrkspaceActivityInRange(
	wrkspaceId: string,
	options: {
		from: Date;
		to: Date;
...
): Promise<ActivityEventDisplay[]> {
	const limit = options.limit ?? 500;
	const conditions = [
		eq(activityEvent.wrkspaceId, wrkspaceId),
		gte(activityEvent.createdAt, options.from),
		lte(activityEvent.createdAt, options.to)
	];

	if (options.actorUserId) {
		conditions.push(eq(activityEvent.actorUserId, options.actorUserId));
	}
	if (options.moduleType) {
		conditions.push(eq(activityEvent.moduleType, options.moduleType));
	}
