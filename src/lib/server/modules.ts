...
import { getDocsModulePreview } from './docs.ts';
import { getForumModulePreview } from './forum.ts';
import { recordActivity, getModuleContext } from './activity.ts';
import { getCardModuleConfig, getCardModuleSettings } from './cards-settings.ts';
import {
	boardRowFromFieldValues,
	buildCardActivitySummary,
	cardFieldValuesFromBoardRow,
	parseCardFieldValuesJson,
	validateCardFieldValues,
	type CardFieldValues,
	type CardModuleConfig
} from '../shared/cards-schema';
import { escapeIlike } from './ilike.ts';
import { getTasksModulePreview } from './tasks.ts';
import { getDecisionsModulePreview } from './decisions.ts';
import { getReportsModulePreview } from './reports.ts';
...
import { DEFAULT_CARD_COLUMN_COLOR } from '../shared/cards';
...
export type ModulePreview =
	...
	| {
			...
			recent: { title: string; summary: string; status: string }[];
	  }
	| ({
			type: 'reports';
	  } & import('$lib/shared/reports-preview').ReportsModulePreviewData)
	| { type: 'other' };
...
async function getDocsPreview(
	userId: string,
	teamSlug: string,
	wrkspaceSlug: string,
	moduleId: string
): Promise<ModulePreview> {
	const { items, totalCount, moreCount } = await getDocsModulePreview(
		userId,
		teamSlug,
		wrkspaceSlug,
		moduleId
	);
	return { type: 'docs', items, totalCount, moreCount };
}
...
async function getDecisionsPreview(moduleId: string): Promise<ModulePreview> {
	const preview = await getDecisionsModulePreview(moduleId);
	return { type: 'decisions', ...preview };
}

async function getReportsPreview(moduleId: string): Promise<ModulePreview> {
	const preview = await getReportsModulePreview(moduleId);
	return { type: 'reports', ...preview };
}
...
export async function listModulesWithPreviews(
	...
	for (const mod of modules) {
		...
		if (mod.type === 'chat') {
			...
			preview = await getDocsPreview(userId, teamSlug, wrkspaceSlug, mod.id);
			...
			preview = await getDecisionsPreview(mod.id);
		} else if (mod.type === 'reports') {
			preview = await getReportsPreview(mod.id);
		}
...
export async function addModule(
	...
	const title = options?.title?.trim() || defaultModuleTitle(type, Number(countRow?.value ?? 0));
	const id = uniqueId();
	const position = (maxPos?.max ?? -1) + 1;
	const creationTemplateId =
		options?.creationTemplateId === undefined ? null : options.creationTemplateId;
...
export async function getCalendarEvent(
	...
	const [row] = await db
		...
		.from(calendarEvent)
		.where(and(eq(calendarEvent.moduleId, moduleId), eq(calendarEvent.id, eventId)));
...
export async function deleteCalendarEvent(
	...
	const { deleteAttachmentsForEvent } = await import('./calendar-attachments.ts');
	const { deleteInvitationsForEvent } = await import('./calendar-invitations.ts');

	await deleteAttachmentsForEvent(eventId);
	await deleteInvitationsForEvent(eventId);
...
export async function ensureCardBoardSeed(
	_userId: string,
	_teamSlug: string,
	_wrkspaceSlug: string,
	_moduleId: string
): Promise<void> {
	// Board setup is handled by the cards setup wizard or wrkspace templates.
}
...
export async function listCardBoard(
	...
	wrkspaceSlug: string,
	moduleId: string
): Promise<{ board: CardBoard; cardModuleConfig: CardModuleConfig }> {
	if (!(await assertCardsModule(userId, teamSlug, wrkspaceSlug, moduleId))) {
		return {
			board: { columns: [] },
			cardModuleConfig: getCardModuleConfig(null)
		};
	}

	const settings = await getCardModuleSettings(moduleId);
	const cardModuleConfig = getCardModuleConfig(settings);
	...
	if (columns.length === 0) {
		return { board: { columns: [] }, cardModuleConfig };
	}
	...
	const cards = await db
		.select({
			...
			body: boardCard.body,
			fieldValues: boardCard.fieldValues,
	...
	for (const card of cards) {
		...
		if (list) {
			const parsedFieldValues = parseCardFieldValuesJson(card.fieldValues);
			list.push({
				id: card.id,
				title: card.title,
				body: card.body,
				fieldValues: cardFieldValuesFromBoardRow(cardModuleConfig, {
					title: card.title,
					body: card.body,
...
export async function deleteCardColumn(
	userId: string,
	teamSlug: string,
	wrkspaceSlug: string,
	moduleId: string,
	...
	const column = await db
		.select({ id: cardColumn.id, title: cardColumn.title })
		.from(cardColumn)
		.where(and(eq(cardColumn.id, columnId), eq(cardColumn.moduleId, moduleId)))
		.limit(1);
...
export async function addBoardCard(
	...
	{ ok: true; cardId: string } | { ok: false; fieldErrors?: { key: string; message: string }[] }
> {
	if (!(await assertCardsModule(userId, teamSlug, wrkspaceSlug, moduleId))) {
		return { ok: false };
	}
	...
	const settings = await getCardModuleSettings(moduleId);
	const config = getCardModuleConfig(settings);
	const validation = validateCardFieldValues(config.schema, rawValues);
	if (!validation.ok) {
		return { ok: false, fieldErrors: validation.fieldErrors };
	}

	const row = boardRowFromFieldValues(config, validation.values);
	const trimmedTitle = row.title.trim();
	if (!trimmedTitle) {
		return {
			ok: false,
			fieldErrors: [{ key: config.layout.primaryFieldKey, message: 'Title is required' }]
		};
	}
...
export async function moveBoardCard(
	...
	const [sourceColumn, targetColumnMeta, movingCard] = await Promise.all([
		db
			.select({ title: cardColumn.title })
			.from(cardColumn)
			.where(eq(cardColumn.id, sourceColumnId))
			.limit(1),
		db
			.select({ title: cardColumn.title })
			.from(cardColumn)
			.where(eq(cardColumn.id, targetColumnId))
			.limit(1),
		db.select({ title: boardCard.title }).from(boardCard).where(eq(boardCard.id, cardId)).limit(1)
	]);
	...
	if (resolved && movingCard[0]) {
		await recordActivity({
			...
			targetId: cardId,
			metadata: {
				title: movingCard[0].title,
				cardTitle: movingCard[0].title,
				fromColumn: sourceColumn[0]?.title ?? 'Column',
				toColumn: targetColumnMeta[0]?.title ?? 'Column'
			}
		});
	}
...
export async function updateBoardCard(
	...
	cardId: string,
	rawValues: Record<string, unknown>
): Promise<{ ok: true } | { ok: false; fieldErrors?: { key: string; message: string }[] }> {
	if (!(await assertCardsModule(userId, teamSlug, wrkspaceSlug, moduleId))) {
		return { ok: false };
	}
	...
	const oldValues = cardFieldValuesFromBoardRow(config, {
		title: existing[0].title,
		body: existing[0].body,
		fieldValues: parseCardFieldValuesJson(existing[0].fieldValues)
	});
	...
	const changes = buildCardActivitySummary(oldValues, validation.values, config.schema);
	const resolved = await resolveWrkspaceAccess(userId, teamSlug, wrkspaceSlug);
...
export async function deleteBoardCard(
	...
	const card = await db
		.select({ id: boardCard.id, columnId: boardCard.columnId, title: boardCard.title })
		.from(boardCard)
		.innerJoin(cardColumn, eq(boardCard.columnId, cardColumn.id))
		.where(and(eq(boardCard.id, cardId), eq(cardColumn.moduleId, moduleId)))
		.limit(1);
