...
import { user, account, team, teamMember, wrkspace, wrkspaceMember } from './db/schema.ts';
...
export async function listAllTeamsWithDetails(): Promise<AdminTeamRow[]> {
	...
	const tmRole = sql.raw('"team_member"."role"');
	const rows = await db
		.select({
			id: team.id,
			name: team.name,
			...
			ownerName: sql<string | null>`(
				...
				WHERE ${teamMember.teamId} = ${teamIdOuter} AND ${tmRole} = 'owner'
				LIMIT 1
			)`,
