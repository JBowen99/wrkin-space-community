import { createDb, type Database } from './connection.ts';

let dbInstance: Database | undefined;
let envOverride: Record<string, string | undefined> | undefined;

export function setDbEnv(source: Record<string, string | undefined>) {
	envOverride = source;
}

function getDbInstance(): Database {
	if (!dbInstance) {
		const url = envOverride?.DATABASE_URL ?? process.env.DATABASE_URL;
