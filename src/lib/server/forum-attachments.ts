...
import { forumPost, forumPostAttachment, forumThread, wrkspaceModule } from './db/schema.ts';
...
} from '../shared/forum-attachments';
import { TASK_ATTACHMENT_ACCEPT } from '../shared/task-attachments';
import { deleteObject, forumAttachmentKey, getObject, putObject } from './storage/index.ts';
...
export async function userCanAccessForumPost(userId: string, postId: string): Promise<boolean> {
	const { userCanAccessForumPostById } = await import('./authorization.ts');
