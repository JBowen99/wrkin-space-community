...
import { filterRecipientsByPreferences, resolveRecipientIds } from './activity.ts';
...
describe('filterRecipientsByPreferences', () => {
	it('excludes the actor', () => {
		...
		expect(filterRecipientsByPreferences(['user-1', 'user-2'], 'tasks', prefs, 'user-1')).toEqual([
			'user-2'
		]);
	});
	...
	it('respects disabled categories', () => {
		...
		expect(filterRecipientsByPreferences(['user-1', 'user-2'], 'tasks', prefs, 'actor')).toEqual([
			'user-2'
		]);
	});
