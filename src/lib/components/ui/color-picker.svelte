...
<Popover.Root bind:open>
	<Popover.Trigger
		...
		aria-expanded={open}
		class="inline-flex items-center justify-between gap-2 rounded-lg border border-border bg-surface-raised text-left text-sm text-ink transition hover:border-accent/40 focus-visible:border-accent focus-visible:ring-2 focus-visible:ring-accent/20 focus-visible:outline-none disabled:cursor-not-allowed disabled:opacity-50 {compact
			? 'h-9 shrink-0 px-2'
			: 'mt-1.5 h-10 w-full px-3 py-2'} {className}"
	>
		...
		<span aria-hidden="true" class="shrink-0 text-ink-muted">▾</span>
	</Popover.Trigger>

	<Popover.Portal>
		<Popover.Content
			class="z-50 w-[15.5rem] rounded-lg border border-border bg-surface-raised p-3 shadow-md"
			sideOffset={6}
			align="start"
		>
			<p class="mb-2 text-xs font-medium text-ink-muted">Presets</p>
			<div class="grid grid-cols-8 gap-1.5" role="listbox" aria-label="Color presets">
				{#each COLOR_PRESETS as preset (preset.value)}
					...
					<button
						...
						title={preset.label}
						class="relative size-7 rounded-md border-2 transition hover:scale-105 focus-visible:ring-2 focus-visible:ring-accent focus-visible:ring-offset-1 focus-visible:outline-none {selected
							? 'border-ink'
							: 'border-transparent'}"
			...
			<div class="mt-3 border-t border-border pt-3">
				<p class="mb-2 text-xs font-medium text-ink-muted">Custom</p>
				...
				<div
					...
					class="relative h-28 w-full cursor-crosshair touch-none overflow-hidden rounded-lg border border-border select-none"
				...
				<div
					...
					class="relative mt-2 h-3 w-full cursor-pointer touch-none rounded-full border border-border select-none"
				...
				<div class="mt-2 flex items-center gap-2">
					<span
						class="size-8 shrink-0 rounded-lg border border-border shadow-inner ring-1 ring-black/5"
						style:background-color={value}
						aria-hidden="true"
					></span>
					<Input
				...
				{#if customError}
					<p class="mt-1 text-xs text-red-600">Enter a valid hex color (e.g. #3b82f6).</p>
				{/if}
			</div>
		</Popover.Content>
	</Popover.Portal>
