		...
		{#if preview.recent.length > 0}
			<ul class="flex min-h-0 flex-1 flex-col gap-1.5 overflow-hidden">
				{#each preview.recent as item, index (index)}
					...
					<li
						class="border-border/80 bg-surface-raised flex min-w-0 items-start gap-2 rounded-lg border border-t-[3px] px-2 py-1.5 shadow-sm"
					...
					>
						...
						<div class="min-w-0 flex-1 py-px">
							...
							<p class="text-ink-muted mt-0.5 truncate text-[10px]">
