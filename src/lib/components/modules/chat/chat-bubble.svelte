	...
	<div class="flex max-w-[75%] flex-col gap-0.5 {isOwn ? 'items-end' : 'items-start'}">
		<div class="flex items-end gap-2 {isOwn ? 'flex-row-reverse' : 'flex-row'}">
			...
			<div class="flex min-w-0 flex-col gap-1">
				{#if !isOwn}
					<span
						class="px-1 text-xs font-medium text-ink-muted {authorMatchKey
							? 'rounded ring-2 ring-amber-400 ring-offset-2 ring-offset-surface'
							: ''}"
				...
				<div class="relative">
					<div
						class="px-3.5 py-2.5 text-sm leading-relaxed {message.reactions.length > 0
							...
							: 'rounded-2xl rounded-bl-sm border border-border bg-surface-raised text-ink'} {bodyMatchKey
					...
					>
						{#if message.body}
							...
							>
								{#if bodySegments}
									<ChatHighlightedText segments={bodySegments} />
								{:else}
									{message.body}
								{/if}
							</p>
					...
					<ChatReactions messageId={message.id} reactions={message.reactions} {isOwn} />
				</div>
				...
				<Tooltip text={formatFullTime(message.createdAt)}>
					{#snippet trigger(props)}
						<time
							{...props}
							class="px-1 text-[0.65rem] text-ink-muted {isOwn ? 'text-right' : 'text-left'}"
