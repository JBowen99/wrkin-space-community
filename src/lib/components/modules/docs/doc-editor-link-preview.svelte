	...
	{#if anchorEl}
		<LinkPreview.Portal>
			<LinkPreview.Content
				...
				align="start"
				sideOffset={8}
				class="z-50 w-72 overflow-hidden rounded-lg border border-border bg-surface-raised shadow-lg"
			...
			>
				...
				<div class="space-y-1 p-3">
					<p class="line-clamp-2 text-sm font-medium text-ink">{displayTitle}</p>
					{#if preview?.description}
						<p class="line-clamp-3 text-xs leading-relaxed text-ink-muted">
							{preview.description}
						</p>
					{:else if loading}
						<p class="text-xs text-ink-muted">Loading preview…</p>
					{/if}
					<p class="truncate pt-1 text-[11px] font-medium text-ink-muted uppercase">
						{displaySite}
					</p>
				</div>
			</LinkPreview.Content>
