<script lang="ts">
	...
	import {
		collabUserColor,
		collabUserFromAwareness,
		type CollabUser
	} from '$lib/shared/collab-user';
	...
	function syncConnectedUsers(provider: HocuspocusProvider | null) {
		...
		async function connect() {
			try {
				...
				provider = new HocuspocusProvider({
					...
					token,
					onSynced() {
						if (cancelled || !el || !provider) return;
						...
						const awareness = provider.awareness;
						if (awareness) {
							const onAwarenessUpdate = () => syncConnectedUsers(provider);
...
<div
	class="doc-editor border-border bg-surface mt-6 flex flex-col overflow-hidden rounded-xl border"
>
	...
	<div class="doc-editor-canvas relative min-h-[28rem] flex-1">
		{#if status === 'loading'}
			<p class="text-ink-muted absolute inset-0 flex items-center justify-center text-sm">
				Connecting…
			</p>
		{:else if status === 'error'}
			<p
				class="text-danger absolute inset-0 flex items-center justify-center px-4 text-center text-sm"
			>
				{statusMessage}
			</p>
		{/if}
		<div
			bind:this={element}
			class="doc-editor-content prose prose-sm text-ink prose-stone prose-headings:text-ink prose-strong:text-ink prose-code:text-ink max-w-none px-6 py-4 focus:outline-none {status !==
			'ready'
				? 'invisible'
				: ''}"
		></div>
