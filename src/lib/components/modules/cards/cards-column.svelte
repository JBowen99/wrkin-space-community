<script lang="ts">
	...
	let { column, columnIndex, config, onCardClick, onAddCard, onEditColumn }: Props = $props();
...
>
	...
	>
				...
				<Tooltip text="Edit column name and color">
					{#snippet trigger(props)}
						<ButtonUi
							...
							type="button"
							variant="unstyled"
							class="group text-ink min-w-0 truncate text-left text-sm font-semibold"
			...
			<div class="flex shrink-0 items-center gap-0.5">
				...
				>
					{#snippet trigger(props)}
						<span
							{...props}
							class="text-ink-muted inline-flex h-5 min-w-5 items-center justify-center rounded-full bg-black/10 px-1.5 text-[10px] font-medium tabular-nums"
						>
							{column.cards.length}
						</span>
					{/snippet}
				...
				<Tooltip text="Add card">
					{#snippet trigger(props)}
						<ButtonUi
							...
							type="button"
							variant="unstyled"
							class="text-ink-muted hover:text-ink flex size-7 items-center justify-center rounded-md transition hover:bg-black/10"
							aria-label="Add card"
		...
		<DndDroppable
			...
			strategy={sortable()}
			accepts="card"
			class="scrollbar-thin flex min-h-0 flex-1 flex-col gap-2 overflow-y-auto p-2"
			spacing={8}
		>
			{#each column.cards as card, cardIndex (card.id)}
				...
				>
					<CardsCard {card} {config} onclick={onCardClick} />
				</DndDraggable>
			{/each}
		</DndDroppable>
	</div>
