...
<div class="attachments">
	<div class="attachments-header">
		<p class="text-sm font-medium text-ink">Attachments</p>
		<input
			bind:this={fileInput}
			type="file"
	...
	{#if attachments.length === 0}
		<p class="text-xs text-ink-muted">No attachments yet.</p>
	{:else}
		<ul class="attachments-list">
			{#each attachments as att (att.id)}
				<li class="attachment-row">
					...
					<span class="text-xs text-ink-muted">{formatAttachmentSize(att.sizeBytes)}</span>
					<ButtonUi
						type="button"
						variant="ghost"
						class="text-red-700"
