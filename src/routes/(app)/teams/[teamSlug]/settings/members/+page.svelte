...
<SettingsSection title="Invite member" description="Send an invite link (valid 7 days).">
	...
	{:else}
		<p class="text-sm text-ink-muted">Invite teammates by adding them directly as members.</p>
	{/if}
</SettingsSection>
					...
					{:else}
						<Tooltip
							text={member.role === 'owner'
								? 'Owners can transfer ownership and delete the team.'
...
>
	...
	to delete the team or transfer ownership. This action cannot be undone by you — only the new owner can
