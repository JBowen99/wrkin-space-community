...
import { getWrkspaceCapabilities, getWrkspaceAccess } from '$lib/server/authorization';
import {
	addModule,
	deleteModule,
...
import { getModuleCatalogEntry, isModuleType } from '$lib/shared/modules';
...
export const load: PageServerLoad = async ({ locals, params }) => {
	...
	const capabilities = access ? getWrkspaceCapabilities(access.effectiveWrkspaceRole) : null;
...
export const actions: Actions = {
	addModule: async ({ request, locals, params }) => {
		...
		const created = await addModule(locals.user!.id, params.teamSlug, params.wrkspaceSlug, type);
