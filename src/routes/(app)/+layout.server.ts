...
import { countUnreadNotifications, listUserNotifications } from '$lib/server/activity';
...
export const load: LayoutServerLoad = async ({ locals, url }) => {
	...
	let wrkspaceCapabilities: ReturnType<typeof getWrkspaceCapabilitiesForAccess> | null = null;
	...
	if (activeTeamSlug && activeWrkspaceSlug) {
		...
		if (access) {
			wrkspaceCapabilities = getWrkspaceCapabilitiesForAccess(access);
		}
	}
