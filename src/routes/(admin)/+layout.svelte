		...
		<div class="mx-auto flex h-14 max-w-6xl items-center justify-between gap-4 px-6">
			...
			<nav class="flex items-center gap-1 text-sm">
				{#each navItems as item}
					<a
						href={item.href}
						class="rounded-lg px-3 py-1.5 font-medium transition-colors {currentPath === item.href || (item.href !== '/admin' && currentPath.startsWith(item.href))
							? 'bg-stone-200/80 text-ink'
