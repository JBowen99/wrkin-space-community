<script lang="ts">
	...
	const inputClass =
		'mt-2 border-transparent bg-surface-muted px-4 py-3 focus:border-stone-300 focus:ring-stone-200/60';
	const passwordFieldClass =
		'border-transparent bg-surface-muted px-4 py-3 focus:border-stone-300 focus:ring-stone-200/60';
...
<div class="flex min-h-screen items-center justify-center bg-surface-muted p-4 sm:p-6">
	<div
		class="flex w-full max-w-5xl flex-col overflow-hidden rounded-[2rem] bg-surface-muted shadow-[0_24px_80px_-12px_rgba(28,25,23,0.12)] ring-1 ring-border/60 lg:min-h-[36rem] lg:flex-row"
	>
		<!-- Form panel -->
		<div class="flex flex-1 flex-col justify-center px-8 py-10 sm:px-12 sm:py-12 lg:px-14 lg:py-14">
			<div class="mx-auto w-full max-w-md lg:mx-0">
				<div
					class="mb-4 rounded-xl border border-warning-muted bg-warning-muted px-4 py-3 text-sm text-warning"
				>
					This is the initial setup for your self-hosted instance. The account you create here will
					be the <strong>admin</strong> — it can manage all users, teams, and spaces from the admin dashboard.
				</div>
				...
				>
					...
					{#if hasError}
						<div
							class="rounded-xl border border-red-200 bg-red-50 px-4 py-3 text-sm text-danger"
							role="alert"
						>
							{errorText}
						</div>
					...
					<Button
						type="submit"
						disabled={submitting}
						class="mt-2 h-12 w-full rounded-xl !bg-ink text-base font-medium hover:!bg-ink-muted disabled:opacity-70"
					>
						{#if submitting}
