...
function isAlreadyExistsError(err: unknown): boolean {
	if (err && typeof err === 'object' && 'code' in err) {
		return ALREADY_EXISTS_CODES.has(String(err.code));
	}
	return false;
}

/**
 * Try running each migration SQL. Records as applied whether it succeeds
 * or the object already exists. Aborts on unexpected errors.
 */
	...
	for (const entry of journal.entries) {
		...
		try {
			...
			if (isAlreadyExistsError(err)) {
				console.log(`skip ${entry.tag} (already exists in schema)`);
			} else {
				throw err;
			}
		}
