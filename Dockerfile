...
RUN node -e "const p=JSON.parse(require('fs').readFileSync('package.json','utf-8'));if(!p.pnpm)p.pnpm={};p.pnpm.onlyBuiltDependencies=['*'];require('fs').writeFileSync('package.json',JSON.stringify(p,null,'\t')+'\n')" && \
    pnpm install
